...
def show_admin_login():
    ...
    st.markdown("<h1 style='text-align: center;'>Administrator Panel</h1>", unsafe_allow_html=True)
    st.markdown("---")
            ...
            for user in teachers:
                ...
                with col_b:
                    if user['username'] not in ['admin']:
                        if st.button("Delete", key=f"del_t_{user['username']}"):
        ...
        if students:
            for user in students:
                ...
                with col_a:
                    st.write(f"**{user['name']}** (@{user['username']}) - {user['email']}")
                with col_b:
                    if st.button("Delete", key=f"del_s_{user['username']}"):
                        success, msg = delete_user(user['username'])
                        if success:
                            st.success(msg)
                            st.rerun()
...
with tab1:
    with st.sidebar:
        ...
        if is_teacher:
            st1, st2, st3 = st.tabs(["PDFs", "Videos", "Manage"])
        else:
    ...
    with ft1:
        ...
        for p in posts:
            ...
            with st.container():
                ...
                with col_a:
                    if st.button(f"Upvote {p['upvotes']}", key=f"up_{p['id']}"):
                        upvote_post(p['id'])
                        st.rerun()
    ...
    if 'viewing_post' in st.session_state:
        ...
        if post:
            ...
            with st.form("reply_form"):
                ...
                with col_back:
                    if st.form_submit_button("Back to Posts"):
                        del st.session_state.viewing_post
                        st.rerun()
        else:
